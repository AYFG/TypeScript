...
(()=>{
    // 타입 단언은 중첩이 가능하다
    const a = 20 as any as string;
    // any는 아무것도 관여하지않지만 unknown은 타입 가드를 통해 안전하게 쓸 수 있다.



    function getMsg(msg:any){ // 리턴 타입은 any
        return msg;
    }
    ...
    const msg1 = getMsg(123.345) as number;
    ...
    function getMsg2<T>(msg:T){
        return msg;
    }
    const msg3 = getMsg2<string>("hello");
